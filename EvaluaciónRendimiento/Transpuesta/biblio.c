...
void inicial_tiempo() { gettimeofday(&start, NULL); }
...
void final_tiempo() {
	...
	printf("\n:-> %9.0f µs\n",
				 (double)(stop.tv_sec * 1000000 +
									stop.tv_usec)); // Imprime el tiempo transcurrido en
																	// microsegundos
}
...
void *mult_thread(void *variables) {
	...
	int ini = (N / nH) * idH; // Calcula el índice inicial para la sección de la
														// matriz a procesar por este hilo
	int fin = (N / nH) * (idH + 1); // Calcula el índice final para la sección de
																	// la matriz a procesar por este hilo
	...
	pthread_mutex_lock(&MM_mutex); // Bloquea el mutex (aunque no parece haber un
																 // uso explícito en este caso)
	pthread_mutex_unlock(&MM_mutex); // Desbloquea el mutex (aunque no parece
