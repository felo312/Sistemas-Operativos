...
static double MEM_CHUNK[DATA_SIZE]; // Tamaño total de la memoria utilizada para las matrices
...
int main(int argc, char *argv[]) {
	...
	mA = MEM_CHUNK; // Asigna la memoria para la matriz A al inicio del bloque de
									// memoria
	mB =
			mA + SZ * SZ; // Asigna la memoria para la matriz B después de la matriz A
	mC =
			mB + SZ * SZ; // Asigna la memoria para la matriz C después de la matriz B
	...
	pthread_mutex_init(&MM_mutex, NULL); // Inicializa el mutex (aunque no parece
																			 // haber un uso explícito en este caso)
	pthread_attr_init(&atrMM);           // Inicializa los atributos del hilo
	pthread_attr_setdetachstate(
			&atrMM, PTHREAD_CREATE_JOINABLE); // Establece los atributos para permitir
																				// la espera de los hilos creados
	...
	for (int j = 0; j < n_threads; j++) {
		...
		pthread_create(&p[j], &atrMM, mult_thread,
									 (void *)datos); // Crea un hilo con los parámetros dados
	}

	for (int j = 0; j < n_threads; j++)
		pthread_join(p[j],
								 NULL); // Espera a que todos los hilos terminen su ejecución
	...
	pthread_mutex_destroy(&MM_mutex); // Destruye el mutex (aunque no parece haber
																		// un uso explícito en este caso)
